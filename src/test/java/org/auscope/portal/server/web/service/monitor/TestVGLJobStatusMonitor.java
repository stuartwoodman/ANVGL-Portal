...
public class TestVGLJobStatusMonitor extends PortalTestClass {
    ...
    private ANVGLUserRepository mockUserRepository;
    private NCIDetailsRepository mockNciRepository;
    ...
    public void init() {
        ...
        mockUserRepository = context.mock(ANVGLUserRepository.class);
        mockNciRepository = context.mock(NCIDetailsRepository.class);
        
        //Component under test
        monitor = new VGLJobStatusMonitor();
        monitor.setJobManager(mockJobManager);
        monitor.setJobStatusMonitor(mockJobStatusMonitor);
        monitor.setJobUserRepository(mockUserRepository);
        monitor.setNciDetailsDao(mockNciRepository);
    }
    ...
    public void testExecuteInternal() throws Exception {
        ...
        context.checking(new Expectations() {{
            ...
            allowing(mockUserRepository).findByEmail(null); will(returnValue(user));
            allowing(mockNciRepository).findByUser(user); will(returnValue(null));
